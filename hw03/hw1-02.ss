...
;--------------------start of HW03-----------------
;Lets take programs from hw02 and evaluate them
...
;UPDATE - It doesnt work so -> lets try this way
;just takes best program from evaluate
;if it is not desired quality, lets do evaluate from
; outputs in hw03 :* :D
;TODO -if it will not work  for 10 points
;lets do that shit with mutation...
(define (evolve pairs threshold stack_size)
  ;(let ((output (car (evaluate my-programs pairs threshold stack_size))))
    (cond
      ;if it is not desired quality -> lets do it on better programs
     ; ((and (= (caar output) 0) (= (cadar output) 0)) output)
      ;launch it on better programs
      (#t (car (evaluate my-programs pairs threshold stack_size)))
      ;TODO - try to mutate or somethin' like that
    )
  )
;)
